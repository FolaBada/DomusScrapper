        ...
        private static async Task ExtractOddsAsync(IPage page, string fileName)
        {
            ...
            for (int i = 0; i < matchCount; i++)
            {
                ...
                try
                {
                    ...
                    if (isTennis || isBasket) hasDropdowns = await row.Locator("button.dropdown-toggle").CountAsync() > 0;
                }
                catch { }

                if (!likelyHasGrid && !(isSoccer ? false : hasDropdowns))
                {
                    ...
                    if (cCount > 0)
                    {
                        for (int j = 0; j < cCount; j++)
                        {
                            ...
                            try
                            {
                                ...
                                if (isSoccer)
                                {
                                    odds[label] = value; // all grid markets
                                }
                                else
                                {
                                    // Basket/Tennis: keep 1/2 only (match-winner) to mirror original Domus payload density
                    ...
                    if (odds.Count == 0 && likelyHasGrid)
                    {
                        ...
                        return JSON.stringify(out);
                            ...
                            foreach (var kv in doc.RootElement.EnumerateObject())
                            {
                                ...
                                if (isSoccer) odds[k] = v;
                                else if (k.Equals("1", StringComparison.OrdinalIgnoreCase) ||
                                         k.Equals("2", StringComparison.OrdinalIgnoreCase))
                                {
                ...
                // (We still parse dropdowns for logging parity, but we DO NOT put them into JSON to keep original Domus shape)
                ...
                if ((doTennisExtractions || doBasketExtractions) /* && hasDropdowns */)
                {
                    var row = match.Locator("xpath=ancestor::div[contains(@class,'contenitoreRiga')]").First;
